...
/**
 * Look up a serial-number entry by the vendor serial (= Sunmi S/N), exact match.
 * The reverse of lookupByRef — used to resolve a scanned Sunmi S/N back to our ref.
 * Returns null if not found.
 * @returns {Promise<{ id: number, ref: string, serial: string|null } | null>}
 */
export async function lookupBySerial(serial, token) {
  try {
    const data = await rentmanGet(`/serialnumbers?serial=${encodeURIComponent(serial)}`, token);
    const item = data.data?.[0];
    if (!item) return null;
    return {
      id:     item.id,
